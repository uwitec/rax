...
public class WareDaoImpl extends SqlMapClientDaoSupport implements
        WareDao {

    @Override
    public int create(Ware newInstance) throws DataAccessException {
        return (Integer) getSqlMapClientTemplate().insert("Ware.create",
                newInstance);
    }

    @Override
    public Ware read(int id) throws DataAccessException {
        return (Ware) getSqlMapClientTemplate().queryForObject(
                "Ware.read", id);
    }

    @Override
    public int update(Ware transientObject) throws DataAccessException {
        return getSqlMapClientTemplate().update("Ware.update",
                transientObject);
    }
